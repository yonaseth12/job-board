...
const LandingPage = () => {
  return (
    <main className="flex flex-col gap-10 sm:gap-20 py-10 sm:py-20">
        <section className='text-center'>
            <h1 className='flex flex-col items-center justify-center gradient gradient-title text-4xl font-extrabold sm:text-6xl lg:text-8xl tracking-tighter py-4'>
                ...
                <span className='flex items-center gap-2 sm:gap-6'>
                    and get{" "}<img src="/logo.png" alt="Company-logo" className="h-14 sm:h-24 lg:h-32"/>
                </span>
            </h1>
        ...
        <Accordion type="single" collapsible className="w-[60%] ml-6">
            {faqs.map((faq, index)=> {
                return (
                    <AccordionItem ket={index} value={`item-${index+1}`}>
